...
Rcpp::List spmeshed_predict(
  ...
  arma::field<arma::mat> w_out_list(ublock.n_elem);
  ...
  for(unsigned int i=0; i<ublock.n_elem; i++){
    ...
    w_out_list(i) = arma::zeros(block_coords.n_rows, nsamples);
...
#pragma omp parallel for 
    ...
    for(unsigned int m=0; m<nsamples; m++){
      ...
      for(unsigned int ix=0; ix<block_coords.n_rows; ix++){
        ...
        double LW_scalar = arma::as_scalar(LW);
        w_out_list(i)(ix,m) = LW_scalar;
  ...
  arma::mat w_out = field_v_concatm(w_out_list);

  
  return Rcpp::List::create(
    Rcpp::Named("preds_out") = preds_out,
    Rcpp::Named("w_out") = w_out,
