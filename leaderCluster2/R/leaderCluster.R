                          ...
                          distance = c("Lp", "L1", "L2", "Linf", "haversine"), p = 2)
{
  ...
  index = index[ord]
  cluster_centroid = matrix(0, nrow = nrow(points), ncol(points))

  time1 = proc.time()
  while (iter <= max_iter )
  {
    cluster_idx = rep(0,nrow(points))
    output = .C("leader_cluster",
              ...
              cluster_centroid = as.double(cluster_centroid))

    out = output$cluster_id + 1
  ...
  clustering.time = proc.time() - time1
  num_clusters = max(cluster_id)
  starting_order = match(1:length(index), index)
  cluster_id = out[starting_order]
  cluster_idx = index[output$cluster_idx[1:num_clusters] + 1]
  cluster_centroids = matrix(output$cluster_centroid,ncol=ncol(points))[1:num_clusters,]
