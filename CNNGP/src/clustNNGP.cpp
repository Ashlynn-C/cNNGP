...
void updateBF(double *B, double *F, double *c, double *C, double *coords, int *nnIndx, int *nnIndxLU, int n, int m, double *distIU, double *distU, int *BIndx, int *FIndx, int Kappa, double sigmaSq, double phi, double nu, int covModel, double *bk, double nuUnifb){
...
extern "C" {
  ...
  SEXP clustNNGP(SEXP y_r, SEXP X_r, SEXP p_r, SEXP n_r, SEXP m_r, SEXP coords_r, SEXP covModel_r, SEXP nnIndx_r, SEXP nnIndxLU_r, SEXP uIndx_r, SEXP uIndxLU_r, SEXP uiIndx_r, 
                 SEXP distIU_r, SEXP distU_r, SEXP BIndx_r, SEXP FIndx_r, SEXP Kappa_r,// new for clustering
    ...
    // args for clustering 
    double *distIU = REAL(distIU_r);
    double *distU = REAL(distU_r);
    int *BIndx = INTEGER(BIndx_r);
    int *FIndx = INTEGER(FIndx_r);
    int Kappa = INTEGER(Kappa_r)[0];
    ...
    int Bsize = static_cast<int>(static_cast<double>(1+m)/2*m+(Kappa-1)*m);

    //other stuff
    int mm = m*m;
    double *B = (double *) R_alloc(Bsize, sizeof(double));
    double *F = (double *) R_alloc(Kappa+m, sizeof(double));
    double *BCand = (double *) R_alloc(Bsize, sizeof(double));
    double *FCand = (double *) R_alloc(Kappa+m, sizeof(double));
    ...
    updateBF(B, F, c, C, coords, nnIndx, nnIndxLU, n, m, distIU, distU, BIndx, FIndx, Kappa, theta[sigmaSqIndx], theta[phiIndx], nu, covModel, bk, nuUnifb);
	...
	if(uIndxLU[n+i] > 0){//is i a neighbor for anybody
	  for(j = 0; j < uIndxLU[n+i]; j++){//how many location have i as a neighbor
		...
		b += B[BIndx[jj]+k]*w[kk];//covariance between jj and kk and the random effect of kk
	      }
	    }
	    aij = w[jj] - b;
	    a += B[BIndx[jj]+uiIndx[uIndxLU[i]+j]]*aij/F[FIndx[jj]];
	    v += pow(B[BIndx[jj]+uiIndx[uIndxLU[i]+j]],2)/F[FIndx[jj]];
	  }
	}
	
	e = 0;
	for(j = 0; j < nnIndxLU[n+i]; j++){
	  e += B[BIndx[i]+j]*w[nnIndx[nnIndxLU[i]+j]];
	}
	mu = (y[i] - F77_NAME(ddot)(&p, &X[i], &n, beta, &inc))/theta[tauSqIndx] + e/F[FIndx[i]] + a;
	
	var = 1.0/(1.0/theta[tauSqIndx] + 1.0/F[FIndx[i]] + v);
...
#pragma omp parallel for private (e, j, b) reduction(+:a, logDet)
	...
	logDet += log(F[FIndx[i]]);
      ...
      updateBF(BCand, FCand, c, C, coords, nnIndx, nnIndxLU, n, m, distIU, distU, BIndx, FIndx, Kappa, theta[sigmaSqIndx], phiCand, nuCand, covModel, bk, nuUnifb);
...
#pragma omp parallel for private (e, j, b) reduction(+:a, logDet)
	...
	if(nnIndxLU[n+i] > 0){
	  e = 0;
	  for(j = 0; j < nnIndxLU[n+i]; j++){
	    e += BCand[BIndx[i]+j]*w[nnIndx[nnIndxLU[i]+j]];
	  }
	  b = w[i] - e;
	}else{
	  b = w[i];
