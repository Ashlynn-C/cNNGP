clustNNGP <- function(formula, data = parent.frame(), coords, method = "response", family="gaussian", weights, n.neighbors = 15, 
                      clust.method = "clustering", radius = NULL, PC = T, PCvar = 0.9, max_iter = 1, clustering.info,
    ...
    ####################################################
    ##Check for Gaussian family and latent model
    ####################################################
    if(family != "gaussian"){stop("error: clustNNGP only supports gaussian family")}
    if(method != "latent"){stop("error: clustNNGP only supports latent method")}
    ...
    if(missing(clustering.info)){
        # points have already been ordered above 
        clustered.distances <- clustering(coords, m = n.neighbors, indx = list(nnIndx = nn.indx, nnIndxLU = nn.indx.lu),
                                          radius = radius, clust.method = clust.method, PC = PC, PCvar = PCvar, max_iter = max_iter, omp.threads = n.omp.threads)
        dist.IU <- clustered.distances$dist.IU
        dist.U <- clustered.distances$dist.U
        BIndx <- clustered.distances$BIndx
        FIndx <- clustered.distances$FIndx
        Kappa <- clustered.distances$k
    } else{
        dist.IU <- clustering.info$dist.IU
        dist.U <- clustering.info$dist.U
        BIndx <- clustering.info$BIndx
        FIndx <- clustering.info$FIndx
        Kappa <- clustering.info$k
    }
    
    storage.mode(dist.IU) <- "double"
    storage.mode(dist.U) <- "double"
    storage.mode(BIndx) <- "integer"
    storage.mode(FIndx) <- "integer"
    storage.mode(Kappa) <- "integer"
    ...
    ptm <- proc.time()
    out <- .Call("clustNNGP", y, X, p, n, n.neighbors, coords, cov.model.indx, nn.indx, nn.indx.lu, u.indx, u.indx.lu, ui.indx,
                                      dist.IU, dist.U, BIndx, FIndx, Kappa, 
